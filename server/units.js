    ...
    if (self.unitTypes.hasOwnProperty(type)) {
      ...
      unit.takeHit = function(hitValue, unit, range) {
        ...
        if (self.type == "base" && self.alive == false) {
          game.winner = unit.player;
          game.active = false;
          mainGame.winner = unit.player;
      ...
      unit.newTurn = function() {
        self.units[unit.id].movesLeft = parseInt(self.units[unit.id].speed);
        self.units[unit.id].actionsLeft = parseInt(self.units[unit.id].actions);
      };
    }
